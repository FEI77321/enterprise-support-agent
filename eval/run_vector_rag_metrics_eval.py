...
from app.vector_store import search_bge_vector_store
...
def evaluate_vector_metrics(
    ...
    for case in positive_cases:
        results = search_bge_vector_store(
            case["query"],
            top_k=top_k,
        )
...
def main() -> None:
    ...
    print()
    print(f"Vector Macro Recall@{TOP_K}: {recall:.4f}")
    print(f"Vector MRR@{TOP_K}: {mrr:.4f}")
    print(f"Vector nDCG@{TOP_K}: {ndcg:.4f}")
